...
def parse_dependencies(root_path):
    ...
    if req_file.exists():
        ...
        if req_file.stat().st_size > 1024 * 1024:
            print(f"Warning: Skipping {req_file} as it exceeds the 1MB size limit.", file=sys.stderr)
        else:
    ...
    for py_file in root_path.rglob("*.py"):
        ...
        try:
            if py_file.stat().st_size > 1024 * 1024:
                print(f"Warning: Skipping {py_file} as it exceeds the 1MB size limit.", file=sys.stderr)
                continue
