...
IGNORE_DIRS = {'.git', '__pycache__', 'node_modules', '.venv', 'venv', '.vscode', 'build', 'dist', 'target'}
...
def analyze_python_code(root_path):
    ...
    # Optimization: Use os.walk instead of rglob to prune directories in-place
    # This prevents scanning massive directories like node_modules or .venv
    for root, dirs, files in os.walk(root_path):
        dirs[:] = [d for d in dirs if d not in IGNORE_DIRS]
