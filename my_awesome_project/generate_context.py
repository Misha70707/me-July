...
def scan_project_structure(root_path):
    ...
    # ⚡ Bolt: Hoist invariant type conversion outside the hot loop
...
def analyze_python_code(root_path):
    ...
    # ⚡ Bolt: Use os.walk with in-place filtering instead of rglob
    # to completely bypass I/O overhead in ignored subdirectories
    for root, dirs, files in os.walk(root_path):
        dirs[:] = [d for d in dirs if d not in ignore_dirs]
        for file in files:
