...
# Limit file size to 1MB to prevent DoS/memory exhaustion
...
def parse_dependencies(root_path):
    ...
    if req_file.exists():
        ...
        if req_file.stat().st_size > MAX_FILE_SIZE:
            print(f"⚠️ Skipping {req_file.name}: File too large ({req_file.stat().st_size} bytes)")
        else:
    ...
    for py_file in root_path.rglob("*.py"):
        ...
        try:
            if py_file.stat().st_size > MAX_FILE_SIZE:
                print(f"⚠️ Skipping {py_file.name}: File too large ({py_file.stat().st_size} bytes)")
                continue
