...
def analyze_python_code(root_path):
    ...
    # Optimization: Use os.walk with pruning instead of rglob to avoid scanning ignored directories
    # like node_modules, .venv, etc. This significantly speeds up scanning in large projects.
    ignore_dirs = {'.git', '__pycache__', 'node_modules', '.venv', 'venv', '.vscode'}

    for root, dirs, files in os.walk(root_path):
        # Prune ignored directories in-place to skip traversal
        dirs[:] = [d for d in dirs if d not in ignore_dirs]
