...
private:
    ...
    vectorf m_ones; // Pre-allocated ones vector
...
public:
    ...
    void Init(int inp, int hid) {
        ...
        // Pre-fill ones vector to avoid allocation in Forward
        m_ones.Resize(hidden_size);
        m_ones.Fill(1.0f);
    ...
    vectorf Forward(vectorf &x, vectorf &h_prev) {
        ...
        // 3. Final State
        // h_new = (1-f) * h_prev + f * h_tilde
        // Optimized: Use pre-allocated m_ones instead of creating new vector
        vectorf forget_part = m_ones - f;
