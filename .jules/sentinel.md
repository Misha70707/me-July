...
**Vulnerability:** Uncontrolled resource consumption due to reading arbitrary-sized files directly into memory using `f.read()`.
**Learning:** Treating file contents as untrusted input is essential, as large files can lead to memory exhaustion and Denial of Service (DoS).
**Prevention:** Enforce a maximum file size limit (e.g., `path.stat().st_size > 1048576`) before reading files into memory.
