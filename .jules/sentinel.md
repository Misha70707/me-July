...
**Vulnerability:** Uncontrolled Resource Consumption (DoS risk) via unbounded file reads using `f.read()`.
**Learning:** File contents must be treated as untrusted input. Reading arbitrarily large files into memory at once can lead to application crashes and denial of service.
**Prevention:** Enforce a file size limit (e.g., `path.stat().st_size > 1048576`) before reading files into memory.
